...
|---|---|---|---|
| `mechanic_assigned` | mechanic | `type`, `repair_request_id`, `assignment_id`, `reference` | Task detail screen (works today) |
| `repair_released` | driver | `type`, `repair_request_id`, `reference` | Repair detail screen (requires §6 driver endpoint) |
| `repair_created` | driver (portal-created only) | `type`, `repair_request_id`, `reference` | Repair detail screen (requires §6 driver endpoint) |
...
> Steps 1–4 hit the **driver** mobile endpoints from §6 — run them only after the backend work is deployed. Steps 5–6 exercise the **mechanic** API, which works today.
