<template>
    <div class="relative" ref="dropdownRef">
        <button @click="toggleDropdown" class="relative p-2 rounded-lg hover:bg-indigo-50 transition-colors">
            <Bell class="w-5 h-5" :class="unreadCount > 0 ? 'text-indigo-600' : 'text-slate-500'" />
        ...
        <transition enter-active-class="transition-all duration-200 ease-out" leave-active-class="transition-all duration-150 ease-in"
            ...
            <div v-if="dropdownOpen" class="absolute right-0 mt-2 w-96 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 max-h-[480px] flex flex-col">
                <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                    <h3 class="text-sm font-semibold text-slate-700">Notifications</h3>
                    <button v-if="unreadCount > 0" @click="markAllRead" class="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Mark all read</button>
                </div>

                <div v-if="loading" class="p-4 space-y-3">
                    <div v-for="i in 3" :key="i" class="flex gap-3 animate-pulse">
                        <div class="w-8 h-8 bg-slate-100 rounded-full flex-shrink-0"></div>
                ...
                <div v-else-if="notifications.length === 0" class="p-8 text-center">
                    <Bell class="w-10 h-10 text-slate-300 mx-auto mb-2" />
                ...
                <div v-else class="overflow-y-auto flex-1">
                    <div v-for="n in notifications" :key="n.id"
                        @click="handleClick(n)"
                        class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 cursor-pointer transition-colors border-b border-slate-50 last:border-0"
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-slate-700 leading-snug" :class="{ 'font-semibold': !n.read_at }">{{ messageFor(n) }}</p>
                            <p class="text-[11px] text-slate-400 mt-1">{{ timeAgo(n.created_at) }}</p>
                        </div>
                    </div>
                </div>

                <div v-if="hasMore" class="border-t border-slate-100 p-2 text-center">
                    <router-link to="/notifications" @click="dropdownOpen = false" class="text-xs text-indigo-600 hover:text-indigo-700 font-medium">View all</router-link>
                </div>
            </div>
        </transition>
    </div>
