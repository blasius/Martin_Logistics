    ...
    <div class="h-screen flex flex-col bg-slate-50 overflow-hidden relative">
        <header class="bg-white border-b border-slate-200 px-8 py-4 flex flex-wrap items-center justify-between gap-4 shadow-sm z-10 no-print">
            <div class="flex items-center gap-4">
                <div class="p-2 bg-indigo-600 rounded-lg"><Calculator class="w-6 h-6 text-white" /></div>
                <div>
                    <h1 class="text-xl font-black text-slate-800 uppercase leading-none">Costing Report</h1>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Revenue vs fuel, maintenance &amp; other costs per trip</p>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-8 py-6 custom-scrollbar">
                ...
                <div v-if="report.summary.total_trips">
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">{{ report.summary.total_distance_km }} km</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Revenue</p>
                                <span class="p-2 rounded-lg bg-emerald-50 text-emerald-600"><TrendingUp class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-emerald-600">{{ fmtMoney(report.summary.total_revenue) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Per delivered trip</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Fuel</p>
                                <span class="p-2 rounded-lg bg-amber-100 text-amber-600"><Droplets class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-amber-600">{{ fmtMoney(report.summary.fuel_amount) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">{{ report.summary.fuel_liters }} liters</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Maintenance</p>
                                <span class="p-2 rounded-lg bg-rose-100 text-rose-600"><Wrench class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-rose-600">{{ fmtMoney(report.summary.repair_amount) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Attributed repairs</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Other / Allowances</p>
                                <span class="p-2 rounded-lg bg-slate-100 text-slate-600"><Receipt class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-slate-700">{{ fmtMoney(report.summary.expense_amount) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Trip expenses</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Cost</p>
                                <span class="p-2 rounded-lg bg-slate-800 text-white"><Wallet class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-slate-900">{{ fmtMoney(report.summary.total_cost) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">{{ fmtMoney(report.summary.cost_per_km) }} / km</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="p-2 rounded-lg" :class="report.summary.total_profit >= 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'"><TrendingUp class="w-4 h-4" /></span>
                            </div>
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Cost / km</p>
                                <span class="p-2 rounded-lg bg-cyan-100 text-cyan-600"><Gauge class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-cyan-600">{{ report.summary.cost_per_km !== null && report.summary.cost_per_km !== undefined ? fmtMoney(report.summary.cost_per_km) : '—' }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">{{ fmtMoney(report.summary.avg_cost_per_trip) }} / trip</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Fuel Used</p>
                                <span class="p-2 rounded-lg bg-teal-100 text-teal-600"><Droplets class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-teal-600">{{ report.summary.fuel_liters }} L</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Total dispensed</p>
                        </div>
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Maintenance (unattributed)</p>
                                <span class="p-2 rounded-lg bg-slate-100 text-slate-600"><Wrench class="w-4 h-4" /></span>
                            </div>
                            <p class="text-xl font-black text-slate-700">{{ fmtMoney(report.summary.unattributed_maintenance) }}</p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Repairs outside trip windows</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                        <div class="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Monthly Revenue vs Total Cost vs Profit</h3>
                            <div class="h-64">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            ...
                            <div v-else class="h-64 flex items-center justify-center">
                                <p class="text-xs font-black text-slate-400 uppercase tracking-widest">No costs recorded</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div v-if="report.top_profitable.length">
                                <div v-for="t in report.top_profitable" :key="t.label" class="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                                    <div>
                                        ...
                                        <span class="text-[9px] font-bold text-slate-400 ml-2">{{ t.trips }} trips · {{ t.distance_km }} km</span>
                                    </div>
                            ...
                            <table class="w-full">
                                <thead class="bg-slate-800 text-white">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Km</th>
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Revenue</th>
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Fuel</th>
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Maint.</th>
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Other</th>
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Total Cost</th>
                                        ...
                                        <th class="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-right">Cost/km</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(row, i) in report.breakdown" :key="row.label" class="border-t border-slate-100 hover:bg-indigo-50/60 transition-colors" :class="row.profit < 0 ? 'bg-rose-50/50' : ''">
                                        ...
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ row.distance_km }}</td>
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ fmtMoney(row.revenue) }}</td>
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ fmtMoney(row.fuel_amount) }}</td>
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ fmtMoney(row.repair_amount) }}</td>
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ fmtMoney(row.expense_amount) }}</td>
                                        <td class="px-4 py-3 text-xs font-bold text-slate-700 text-right whitespace-nowrap">{{ fmtMoney(row.total_cost) }}</td>
                                        ...
                                        <td class="px-4 py-3 text-xs text-slate-600 text-right whitespace-nowrap">{{ row.cost_per_km !== null ? fmtMoney(row.cost_per_km) : '—' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div v-else class="flex flex-col items-center justify-center py-24 gap-3">
                <Calculator class="w-10 h-10 text-slate-300" />
...
import { TrendingUp, TrendingDown, Download, RefreshCw, Truck, Wallet, Receipt, Percent, Droplets, Wrench, Gauge, Calculator } from 'lucide-vue-next';
...
const generate = async () => {
    ...
    try {
        ...
        const { data } = await reportsApi.costing(params);
        report.value = data;
    } catch (e) {
...
const renderCharts = () => {
        ...
        trendChart = new Chart(trendCanvas.value, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { label: 'Total Cost', data: trend.map((t) => t.total_cost), backgroundColor: 'rgba(245, 158, 11, 0.85)', borderRadius: 4 },
    ...
    if (costCanvas.value && report.value.cost_breakdown.length) {
        ...
        const palette = ['#f59e0b', '#ef4444', '#64748b', '#6366f1', '#10b981', '#8b5cf6', '#0ea5e9', '#f97316', '#14b8a6', '#ec4899'];
...
const exportCsv = () => {
    ...
    const headers = ['Label', 'Trips', 'Km', 'Revenue', 'Fuel', 'Maintenance', 'Other', 'Total Cost', 'Profit', 'Margin %', 'Cost/km'];
    const csv = [
        headers.join(','),
        ...rows.map((r) => [r.label, r.trips, r.distance_km, r.revenue, r.fuel_amount, r.repair_amount, r.expense_amount, r.total_cost, r.profit, r.margin, r.cost_per_km ?? '']
    ...
    a.href = url;
    a.download = `costing_${filters.group_by}_${filters.from}_${filters.to}.csv`;
